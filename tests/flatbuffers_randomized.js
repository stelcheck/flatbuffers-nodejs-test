...
function generate() {
	...
	for (var i = 0; i < len; i += 1) {
		var name = builder.createString('Holtzinger ' + i);
		schema.Monster.startMonster(builder);
		schema.Monster.addPos(builder, schema.Vec3.createVec3(builder, 1 * i, 2 * i, 3 * i))
		schema.Monster.addMana(builder, i * 4);
		schema.Monster.addHp(builder, i * 5);
		schema.Monster.addName(builder, name);
		schema.Monster.addColor(builder, i % 3 + 1);
